const InventoryService = {
   ...
   */
  filterInventoryRecordsByClosestDate(allRecords, targetYear, targetMonth) {
   ...
   */
  getInventoryRecords(year, month) {
    ...
    return this.filterInventoryRecordsByClosestDate(allRecords, year, month);
  },
   ...
   */
  getInventoryRecordsJson(year, month) {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const sheet = ss.getSheetByName("Inventory_Records");
    if (!sheet) throw new Error("Inventory_Recordsシートが見つかりません。");

    const allRecords = sheet.getDataRange().getValues();
    const filteredRecords = this.filterInventoryRecordsByClosestDate(allRecords, year, month);
   ...
   */
  exportInventoryRecordsCsv(year, month) {
    const records = this.getInventoryRecords(year, month);
   ...
   */
  exportInventoryRecordsExcel(year, month) {
    const records = this.getInventoryRecords(year, month);
    if (records.length <= 1) {
   ...
   */
  exportInventoryRecordsPdf(year, month) {
    const records = this.getInventoryRecords(year, month);
    if (records.length <= 1) {
      throw new Error("出力する棚卸記録がありません。");
    }
