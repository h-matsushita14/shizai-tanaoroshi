...
const LocationService = {
  ...
  getLocationsMaster(): LocationMasterData[] {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  ...
  addLocation(locationData: LocationMasterData): { message: string; location: LocationMasterData } {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const sheet = ss.getSheetByName("Location_Master");
    if (!sheet) throw new Error("Location_Masterシートが見つかりません。");
  ...
  editLocation(locationData: LocationMasterData): { message: string; location: LocationMasterData } {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const sheet = ss.getSheetByName("Location_Master");
    if (!sheet) throw new Error("Location_Masterシートが見つかりません。");
  ...
  deleteLocation(locationId: string): { message: string; locationId: string } {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const sheet = ss.getSheetByName("Location_Master");
    if (!sheet) throw new Error("Location_Masterシートが見つかりません。");
