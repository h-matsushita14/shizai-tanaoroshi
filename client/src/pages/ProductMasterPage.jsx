  ...
  const [isDetailsDialogOpen, setIsDetailsDialogOpen] = useState(false);
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        const fullUrl = `${scriptUrl}?action=getProducts`;
        console.log("Fetching from URL:", fullUrl); // Debugging line
        ...
        if (result.status === 'success') {
          // Google Apps Scriptから返されるデータにはidがないため、ここで生成する
    ...
    const columns = [
      ...
      { field: '仕入先名', headerName: '仕入先名', width: 150 }, // 新しく追加
    ...
    return (
      <Box sx={{ p: 3 }}>
        ...
        <div style={{ height: 600, width: '100%' }}>
          <DataGrid
            rows={filteredProducts} // フィルターされた商品リストを使用
            columns={columns}
